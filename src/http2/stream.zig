...
pub const Windows = struct {
    send: flow.StreamSendWindow = .{},
    receive: flow.FlowWindow,

    pub fn init(local_initial_window: u31) Windows {
        return .{
            .send = .{},
    ...
    pub inline fn consumeSend(self: *Windows, peer_initial_window: u31, amount: u32) error{FlowControl}!void {
        try self.send.consume(peer_initial_window, amount);
    }

    pub inline fn consumeSendAssumeAvailable(self: *Windows, amount: u32) void {
        self.send.consumeAssumeAvailable(amount);
    }
    ...
    pub inline fn peerWindowUpdate(self: *Windows, peer_initial_window: u31, increment: u31) error{FlowControl}!void {
        try self.send.update(peer_initial_window, increment);
    }
...
pub const Tracked = struct {
    ...
    pub fn init(local_initial_window: u31) Tracked {
        return .{ .windows = Windows.init(local_initial_window) };
    }
};
...
test "stream windows track directional credit" {
    var windows = Windows.init(80_000);
    try windows.consumeSend(70_000, 10_000);
    try windows.receiveData(20_000);
    try std.testing.expectEqual(@as(u31, 60_000), windows.send.available(70_000));
    try std.testing.expectEqual(@as(u31, 60_000), windows.receive.available());
    try windows.peerWindowUpdate(70_000, 2_000);
    try windows.creditReceive(3_000);
    try std.testing.expectEqual(@as(u31, 62_000), windows.send.available(70_000));
    try std.testing.expectEqual(@as(u31, 63_000), windows.receive.available());
}

test "stream send window observes peer initial size without mutation" {
    var windows = Windows.init(65_535);
    try windows.consumeSend(65_535, 10_000);
    try std.testing.expectEqual(@as(u31, 55_535), windows.send.available(65_535));
    try std.testing.expectEqual(@as(u31, 22_768), windows.send.available(32_768));
}
