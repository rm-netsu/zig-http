...
`parseRequest` and `parseResponse` validate fields and determine body framing in one traversal. When the head spans reads, use `HeadParser.feedRequest` or `HeadParser.feedResponse`; only head bytes are copied to caller-owned scratch storage.
...
- `http2/frame.zig` — contiguous and incremental zero-copy frame parsing plus frame serialization.
