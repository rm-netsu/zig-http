...
pub fn main(init: std.process.Init) !void {
        ...
        while (stream_index < common.streams_per_connection) : (stream_index += 1) {
            ...
            if (body == 0) {
                ...
                while (left != 0) {
                    ...
                    const prepared = http.http2.dispatch.prepare(&conn, peer.settings.initial_window_size, complete);
                    const work = switch (prepared) {
                        .data => |work| work,
    ...
    common.print("dispatch prepare + detached DATA", @intCast(now(io) - start), frames);
}
