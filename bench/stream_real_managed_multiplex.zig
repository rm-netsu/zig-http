...
pub fn main(init: std.process.Init) !void {
    ...
    while (round < common.rounds) : (round += 1) {
        ...
        if (manager.activeLocal() != common.streams_per_connection) return error.BadActiveCount;
        ...
        while (order < common.streams_per_connection) : (order += 1) {
            ...
            if (body == 0) {
                ...
                while (left != 0) {
                    ...
                    try cursor.creditReceive(@intCast(n));
                }
            }
            store.remove(id);
            tx += 1;
        }
        if (manager.activeLocal() != 0) return error.BadActiveCount;
    }
    common.print("stream managed multiplex lifecycle", @intCast(now(io) - start), tx);
