...
pub fn main(init: std.process.Init) !void {
    ...
    for (0..shared.rounds) |round| {
        ...
        while (left != 0) {
            ...
            const result = try ctx.session.sendData(&ctx.store, &discard.writer, id, chunk, left <= chunk.len);
    ...
    shared.print("send session managed", elapsed, tx, fields_total, wire_bytes);
}
