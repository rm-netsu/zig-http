...
pub fn main(init: std.process.Init) !void {
    ...
    while (round < common.rounds) : (round += 1) {
        ...
        while (stream_index < common.streams_per_connection) : (stream_index += 1) {
            ...
            while (left != 0) {
                const offer = try detached.dataSendOffer(peer.settings.initial_window_size);
                const grant = try http.http2.dispatch.grantDataSend(&peer, offer);
                if (grant.max_payload == 0) return error.FlowControl;
                const amount: u31 = @intCast(@min(@as(u32, grant.max_payload), left));
                left -= amount;
                try peer.consumeSend(amount);
                const effect = offer.commit(detached, amount, left == 0);
    ...
    common.print("offer/grant DATA credit", @intCast(now(io) - start), chunks);
}
